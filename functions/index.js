...
async function assertOwnerAccountActive(ownerId) {
  ...
  if (isArchivedStatus(ownerSnap.data()?.status)) {
    throw new HttpsError("failed-precondition", "Owner account is archived.");
  }
}
...
async function assertPlateNotInUse(plateNumber, options = {}) {
  ...
  if (!activeSessionSnap.empty) {
    throw new HttpsError("already-exists", "This plate already has an active parking session.");
  }
  ...
  if (hasReservedConflict) {
    throw new HttpsError("already-exists", "This plate already has an active reservation.");
  }
}
...
exports.createBooking = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  const now = nowMs();
  const startTimeMs = toNumber(request.data?.startTimeMs, now);
  const endTimeMs = toNumber(request.data?.endTimeMs, now + 60 * 60 * 1000); // default 1 hour duration
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!userSnap.exists) throw new HttpsError("not-found", "Driver profile not found.");
    ...
    if (currentBalance < reservationFee) {
      throw new HttpsError("failed-precondition", `Insufficient wallet balance. Hold fee is ${reservationFee} ETB, but your balance is ${currentBalance} ETB. Please top up.`);
    }
    ...
    if (!assignedSpot) {
      throw new HttpsError("resource-exhausted", "No specific spots are available for this reservation window.");
    }
...
exports.checkInVehicle = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  const parkingId = String(request.data?.parkingId || "").trim();
  const plateNumber = normalizePlate(request.data?.plateNumber);
  const reservationCode = String(request.data?.reservationCode || "").trim();
  ...
  if (!parkingId || !finalPlateNumber) {
    throw new HttpsError("invalid-argument", "parkingId and plateNumber (or reservationCode) are required.");
  }
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!bookingId) {
      if (!allowWalkIn) {
        throw new HttpsError("failed-precondition", "No active reservation found. Enable walk-in check-in to proceed.");
      }
      if (toNumber(parking.availableSlots) <= 0) {
        throw new HttpsError("resource-exhausted", "No available slots for walk-in check-in.");
      }

      assignedSpot = await allocateSpot(parkingId, now, now + 24 * 60 * 60 * 1000, parking.slotCapacity);
      if (!assignedSpot) {
        throw new HttpsError("resource-exhausted", "No specific spots are currently available.");
      }
...
exports.checkOutVehicle = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  const parkingId = String(request.data?.parkingId || "").trim();
  const plateNumber = normalizePlate(request.data?.plateNumber);
  const paymentMethod = String(request.data?.paymentMethod || "cash").trim().toLowerCase(); // "cash" or "wallet"
  ...
  if (activeSessionQuery.empty) {
    throw new HttpsError("not-found", "No active session found for this plate number.");
  }
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (paymentMethod === "wallet") {
      if (!session.driverId) {
        throw new HttpsError("failed-precondition", "This session was a walk-in without a registered driver. Cannot deduct from wallet. Please pay with cash.");
      }
      ...
      if (currentBalance < amountDue) {
        throw new HttpsError("failed-precondition", `Insufficient driver wallet balance. Fee due is ${amountDue} ETB, but driver's balance is ${currentBalance} ETB. Please pay with cash.`);
      }
...
exports.driverCheckOutVehicle = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    const currentBalance = toNumber(userSnap.data().walletBalance, 100.00);
    if (currentBalance < amountDue) {
      throw new HttpsError("failed-precondition", `Insufficient wallet balance. Total due is ${amountDue} ETB, but your balance is ${currentBalance} ETB. Please top up.`);
    }
...
exports.topUpWallet = onCall(CALLABLE_OPTIONS, async (request) => {
  const actorUid = request.auth?.uid;
  const profile = await requireRole(request, "driver");

  const amount = toNumber(request.data?.amount, 0);
  if (amount <= 0) {
...
exports.expireBookingsManual = onCall(CALLABLE_OPTIONS, async (request) => {
  const now = ts();
  const snapshot = await db
    .collection("bookings")
    .where("status", "==", "reserved")
...
exports.getOwnerAnalytics = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  if (isArchivedStatus(ownerAccount.status)) {
    throw new HttpsError("failed-precondition", "Owner account is archived. Contact admin to restore access.");
  }
  const parkings = parkingsSnap.docs.map((doc) => ({ id: doc.id, ...doc.data() }));
...
exports.adminArchiveOwner = onCall(CALLABLE_OPTIONS, async (request) => {
  const actorUid = request.auth?.uid;
  await requireRole(request, "admin");
  ...
  if (!ownerUserRef || !ownerUserSnap?.exists) {
    throw new HttpsError("failed-precondition", "Owner auth profile was not found.", {
  ...
  if (String(ownerUser.role || "").trim().toLowerCase() !== "owner") {
    throw new HttpsError("failed-precondition", "Target user is not an owner.", {
      reason: OWNER_ARCHIVE_REASONS.INVALID_OWNER_ROLE,
      ownerId,
    });
  }
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!freshOwnerSnap.exists) throw new HttpsError("not-found", "Owner not found.");
    if (!freshOwnerUserSnap.exists) throw new HttpsError("not-found", "Owner auth profile not found.");
...
exports.adminRestoreOwner = onCall(CALLABLE_OPTIONS, async (request) => {
  const actorUid = request.auth?.uid;
  await requireRole(request, "admin");
...
exports.upsertParking = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  if (isArchivedStatus(ownerSnap.data()?.status)) {
    throw new HttpsError("failed-precondition", "Cannot create or update parking for an archived owner.");
  }
...
exports.assignOperatorToParking = onCall(CALLABLE_OPTIONS, async (request) => {
  ...
  await db.runTransaction(async (tx) => {
    ...
    if (!parkingOwnerId) {
      throw new HttpsError("failed-precondition", "Parking ownerId is missing.");
    }
    const ownerRef = db.collection("owners").doc(parkingOwnerId);
    const ownerSnap = await tx.get(ownerRef);
    if (!ownerSnap.exists) {
      throw new HttpsError("not-found", "Parking owner not found.");
    }
    if (assign && isArchivedStatus(ownerSnap.data()?.status)) {
      throw new HttpsError("failed-precondition", "Cannot assign operators to an archived owner.");
    }
